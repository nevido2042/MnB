...
void UInventoryUserWidget::NativeConstruct()
{
	...
	HeadSlot = Cast<UItemSlotUserWidget>(GetWidgetFromName(TEXT("UI_HeadSlot")));
	HeadSlot->SetImage(nullptr);

	EquipmentSlotBindEvent();
...
void UInventoryUserWidget::FlushInven()
{
	...
	{
		HeadSlot->Image->SetBrushFromTexture(nullptr, false);
	}
...
void UInventoryUserWidget::EquipmentSlotBindEvent()
{
	...
	HeadSlot->ItemBtnClicked.BindLambda([this](UItemSlotUserWidget* InSlot)
		{
			InventorySubsystem->UnEquipHead(this);
		});
}
